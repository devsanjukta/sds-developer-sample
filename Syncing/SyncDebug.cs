...
namespace DeveloperSample.Syncing
{
    public class SyncDebug
    {
        ...
        public Dictionary<int, string> InitializeDictionary_Prev(Func<int, string> getItem)
        {
        ...
        // Fixed version of InitializeDictionary
        // This runs getItem ONCE for each key, safely, in parallel
        public Dictionary<int, string> InitializeDictionary(Func<int, string> getItem)
        {
            var itemsToInitialize = Enumerable.Range(0, 100).ToList();
            var concurrentDictionary = new ConcurrentDictionary<int, string>();

            // Parallel.ForEach splits the work among threads, each item is handled once, by any thread.
            Parallel.ForEach(itemsToInitialize, item =>
            {
                // TryAdd means "add once only"
